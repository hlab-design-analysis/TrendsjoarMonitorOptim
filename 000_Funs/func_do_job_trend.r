	...
	do_job_trend<-function(lo, ls1, years){
      ...
      out<-rbindlist(lapply(ls1, function(x, sampfreq1 = sampfreq) {
        # selects years
        # samples from 1:sampfreq1 to select the starting point of the series
        # samples series with the same length [eg, if years 2002:2018 (i.e., 17 years) and sampfreq = 2 samples 8 years;
         if(sampfreq1 %in% c(2,3)) 
           {
           years_index<-seq(sample(1:sampfreq1,1), length(unique(x$Year))%/%sampfreq1*sampfreq1, by = sampfreq1) 
           } else {
        ...
        # stores results 
        data.table(SampSize = x$SampSize[1], variable = x$variable[1], sampfreq = sampfreq1, years = paste(dt2$Year, collapse=","), 
